...
struct ImageBuffer {
  ...
  const char* buffer;
  /**
   * The buffer length of the image, in bytes.
   */
  unsigned int length;
  /**
   * The actual width (px) of the image.
   */
  unsigned int width;
  /**
   * The actual height (px) of the image.
   */
  unsigned int height;

  /**
   * The format of the image buffer. Just support VIDEO_PIXEL_I420 / VIDEO_PIXEL_BGRA / VIDEO_PIXEL_NV21 / VIDEO_PIXEL_RGBA / VIDEO_PIXEL_NV12
   */
  agora::media::base::VIDEO_PIXEL_FORMAT format;

  ImageBuffer() : buffer(NULL), length(0), width(0), height(0),format(agora::media::base::VIDEO_PIXEL_RGBA) {}
};
