...
/**
 * The definition of the EncodedVideoFrameInfoBase struct, which contains the information of the external encoded video frame.
 */
struct EncodedVideoFrameInfoBase {
  EncodedVideoFrameInfoBase()
    : codecType(VIDEO_CODEC_H264),
      width(0),
      height(0),
      framesPerSecond(0),
  ...
  EncodedVideoFrameInfoBase(const EncodedVideoFrameInfoBase& rhs)
  ...
  EncodedVideoFrameInfoBase& operator=(const EncodedVideoFrameInfoBase& rhs) {
...
struct VideoTrackInfo {
  ...
  uid_t ownerUid;
   /**
   * user account of the user who publishes the video track.
   */
  util::AString ownerUserAccount;
  /**
...
struct AudioVolumeInfoBase {
  ...
  AudioVolumeInfoBase() : volume(0), vad(0), voicePitch(0.0) {}
};
 ...
 */
struct TranscodingVideoStreamBase {
  /**
 ...
 */
struct LocalTranscoderConfigurationBase {
  /**
  ...
  LocalTranscoderConfigurationBase() : videoOutputConfiguration(), syncWithPrimaryCamera(true) {}
};
...
struct VideoCanvasBase {
  ...
  VideoCanvasBase()
    : view(NULL), backgroundColor(0x00000000), renderMode(media::base::RENDER_MODE_HIDDEN), mirrorMode(VIDEO_MIRROR_MODE_AUTO),
      setupMode(VIDEO_VIEW_SETUP_REPLACE), sourceType(VIDEO_SOURCE_CAMERA_PRIMARY), mediaPlayerId(-ERR_NOT_READY),
      cropArea(0, 0, 0, 0), enableAlphaMask(false), position(media::base::POSITION_POST_CAPTURER),subviewUid(0) {}

  VideoCanvasBase(view_t v, media::base::RENDER_MODE_TYPE m, VIDEO_MIRROR_MODE_TYPE mt)
    : view(v), backgroundColor(0x00000000), renderMode(m), mirrorMode(mt), setupMode(VIDEO_VIEW_SETUP_REPLACE),
      sourceType(VIDEO_SOURCE_CAMERA_PRIMARY), mediaPlayerId(-ERR_NOT_READY),
      cropArea(0, 0, 0, 0), enableAlphaMask(false), position(media::base::POSITION_POST_CAPTURER),subviewUid(0) {}

  VideoCanvasBase(view_t v, media::base::RENDER_MODE_TYPE m, VIDEO_MIRROR_MODE_TYPE mt,uid_t subviewUid)
    : view(v), backgroundColor(0x00000000), renderMode(m), mirrorMode(mt), setupMode(VIDEO_VIEW_SETUP_REPLACE),
      sourceType(VIDEO_SOURCE_CAMERA_PRIMARY), mediaPlayerId(-ERR_NOT_READY),
      cropArea(0, 0, 0, 0), enableAlphaMask(false), position(media::base::POSITION_POST_CAPTURER),subviewUid(subviewUid) {}
};
...
/** The definition of ChannelMediaInfoBase.
 */
struct ChannelMediaInfoBase {
  ...
  ChannelMediaInfoBase() : channelName(NULL), token(NULL) {}
  ChannelMediaInfoBase(const char* c, const char* t) : channelName(c), token(t) {}
};

/** The definition of ChannelMediaRelayConfigurationBase.
 */
struct ChannelMediaRelayConfigurationBase {
  ...
  ChannelMediaRelayConfigurationBase() : destCount(0) {}
};
 ...
 */
struct RecorderStreamInfoBase {
    /**
     * The channel ID of the audio/video stream needs to be recorded.
     */
    const char* channelId;
    RecorderStreamInfoBase() : channelId(NULL){}
    RecorderStreamInfoBase(const char* channelId) : channelId(channelId) {}
};
