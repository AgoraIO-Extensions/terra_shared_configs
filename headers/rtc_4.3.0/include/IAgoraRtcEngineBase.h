...
struct VideoCompositingLayoutBase {
  struct RegionBase {
 ...
 */
class IRtcEngineEventHandlerBase {
 public:
  virtual ~IRtcEngineEventHandlerBase() {}

  virtual const char* eventHandlerType() const { return "event_handler_base"; }
...
struct RtcEngineContextBase {
  ...
  RtcEngineContextBase()
      : appId(NULL), context(NULL), channelProfile(CHANNEL_PROFILE_LIVE_BROADCASTING),
...
*/
class IMetadataObserverBase {
public:
    virtual ~IMetadataObserverBase() {}
    ...
    struct MetadataBase
    {
        ...
        MetadataBase() : size(0), buffer(NULL), timeStampMs(0) {}
    };
...
struct ExtensionInfoBase {
  ...
  ExtensionInfoBase() : mediaSourceType(agora::media::UNKNOWN_MEDIA_SOURCE), channelId(NULL) {}
};

/**
 * The IRtcEngineBase class, which is the basic interface of the Agora SDK that implements the core functions of real-time communication.
 *
 * `IRtcEngineBase` provides the main methods that your app can call.
 *
 */
class IRtcEngineBase : public agora::base::IEngineBase {
 public:
  /**
