...
class IRtcEngineEventHandlerS : virtual public IRtcEngineEventHandlerBase {
 public:
   ...
   */
  virtual void onAudioVolumeIndication(const AudioVolumeInfoS* speakersS, unsigned int speakerNumber,
                                       int totalVolume) {
    (void)speakersS;
    (void)speakerNumber;
    (void)totalVolume;
  ...
  */
   ...
   */
  virtual void onRemoteAudioStats(const RemoteAudioStatsS& statsS) {
    (void)statsS;
  }
   ...
   */
  virtual void onLocalVideoStats(VIDEO_SOURCE_TYPE source, const LocalVideoStatsS& statsS) {
    (void)source;
    (void)statsS;
  }
   ...
   * RemoteVideoStats.
   */
  virtual void onRemoteVideoStats(const RemoteVideoStatsS& statsS) {
    (void)statsS;
  }
   ...
   */
  virtual void onLocalVideoTranscoderError(const TranscodingVideoStreamS& streamS, VIDEO_TRANSCODER_ERROR error){
    (void)streamS;
    (void)error;
  }
};
...
public:
    ...
    /** Occurs when the local user receives the metadata.
     ...
     - false: Do not send.
     */
    virtual bool onReadyToSendMetadata(MetadataS &metadataS, VIDEO_SOURCE_TYPE source_type) = 0;
     ...
     */
    virtual void onMetadataReceived(const MetadataS& metadataS) = 0;
};
...
class IRtcEngineS : virtual public IRtcEngineBase {
 public:
   ...
   * - < 0: Failure.
   */
  virtual int initialize(const RtcEngineContextS& contextS) = 0;
   ...
   * @param uidNumber The number of userAccount in uidList.
   *
   * @note
   * If userAccount is in uidList, the remote user's audio will not be subscribed,
   ...
   *
   * @note
   * If userAccount is in uidList, the remote user's audio will be subscribed,
   ...
   *
   * @note
   * If userAccount is in uidList, the remote user's video will not be subscribed,
   ...
   *
   * @note
   * If userAccount is in uidList, the remote user's video will be subscribed,
   ...
   * - < 0: Failure.
   */
  virtual int destroyMediaRecorder(agora_refptr<IMediaRecorderS> mediaRecorderS) = 0;
   ...
   * - < 0: Failure.
   */
  virtual int registerAudioSpectrumObserver(agora::media::IAudioSpectrumObserverS* observerS) = 0;
  /**
   ...
   * - < 0: Failure.
   */
  virtual int unregisterAudioSpectrumObserver(agora::media::IAudioSpectrumObserverS* observerS) = 0;
   ...
   * - < 0: Failure.
   */
  virtual int enableExtension(const char* provider, const char* extension, const ExtensionInfoS& extensionInfoS, bool enable = true) = 0;
   ...
   * - < 0: Failure.
   */
  virtual int setExtensionProperty(const char* provider, const char* extension, const ExtensionInfoS& extensionInfoS, const char* key, const char* value) = 0;
   ...
   * - < 0: Failure.
   */
  virtual int getExtensionProperty(const char* provider, const char* extension, const ExtensionInfoS& extensionInfoS, const char* key, char* value, int buf_len)  = 0;
  ...
  /** Publishes the local stream with transcoding to a specified CDN live RTMP address.  (CDN live only.)
    ...
    */
  virtual int startRtmpStreamWithTranscoding(const char* url, const LiveTranscodingS& transcodingS) = 0;
  ...
  /** Update the video layout and audio settings for CDN live. (CDN live only.)
    ...
    * - < 0: Failure.
    */
  virtual int updateRtmpTranscoding(const LiveTranscodingS& transcodingS) = 0;

  virtual int startLocalVideoTranscoder(const LocalTranscoderConfigurationS& configS) = 0;
  virtual int updateLocalTranscoderConfiguration(const LocalTranscoderConfigurationS& configS) = 0;

  // The following APIs are not implemented yet.
  virtual bool registerEventHandler(IRtcEngineEventHandlerS* eventHandlerS) = 0;
  virtual bool unregisterEventHandler(IRtcEngineEventHandlerS* eventHandlerS) = 0;
  ...
  /** Registers the metadata observer.
   ...
   - < 0: Failure.
  */
  virtual int registerMediaMetadataObserver(IMetadataObserverS *observerS, IMetadataObserverS::METADATA_TYPE type) = 0;
  ...
  /** Unregisters the metadata observer.
   ...
   - < 0: Failure.
  */
  virtual int unregisterMediaMetadataObserver(IMetadataObserverS* observerS, IMetadataObserverS::METADATA_TYPE type) = 0;
...
class AAudioDeviceManagerS : public agora::util::AutoPtr<IAudioDeviceManager> {
 public:
  AAudioDeviceManagerS(IRtcEngineS* engineS) {
    queryInterface(engineS, AGORA_IID_AUDIO_DEVICE_MANAGER);
  }
};

class AVideoDeviceManagerS : public agora::util::AutoPtr<IVideoDeviceManager> {
 public:
  AVideoDeviceManagerS(IRtcEngineS* engineS) {
    queryInterface(engineS, AGORA_IID_VIDEO_DEVICE_MANAGER);
  }
};
