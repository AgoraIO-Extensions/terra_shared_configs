...
enum STREAM_FALLBACK_OPTIONS {
  /** 0: No fallback operation to a lower resolution stream when the network
     condition is poor. Fallback to a lower temporal layer (e.g. in a 3-layer SVC)
  ...
  /** 3~8: If the receiver SDK uses agora::rtc::ENABLE_SIMULCAST_MULTI_LAYER in NASA2，it will receive
     one of the streams from agora::rtc::VIDEO_STREAM_LAYER_1 to agora::rtc::VIDEO_STREAM_LAYER_6
     if the related layer exists when the network condition is poor. The lower bound of fallback depens on
