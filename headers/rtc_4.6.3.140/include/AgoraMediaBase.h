...
enum RecorderReasonCode {
  ...
  /**
   * 5: The recording failed to start.
   */
  RECORDER_REASON_FAILED = 5,
  /**
   * 6: Failed to initialize the recorder.
   */
  RECORDER_REASON_INITIALIZE_FAILED = 6,
  /**
   * 7: Failed to setup video track.
   */
  RECORDER_REASON_SETUP_VIDEO_TRACK_FAILED = 7,
  /**
   * 8: Failed to setup audio track.
   */
  RECORDER_REASON_SETUP_AUDIO_TRACK_FAILED = 8,
  /**
   * 9: Failed to start recording after setup.
   */
  RECORDER_REASON_START_FAILED = 9,
  /**
   * 10: Invalid state (e.g., trying to start preview recording when already connected).
   */
  RECORDER_REASON_INVALID_STATE = 10,
};
/**
 * Recording content. Set in `startAudioRecording`.
 */
 enum AUDIO_RECORDING_CONTENT_TYPE {
  /**
   ...
   */
  REMOTE_AUDIOS_ONLY = 2,
  /**
   * 3: Records the mixed audio of the local and all remote users.
   */
  MIXED_ALL_AUDIOS = 3,
  /**
   * 4: Records all audio streams in the channel.
   */
  ALL_AUDIO_STREAMS = 4,
  /**
   * 5: Records the local upstream mixed audio.
   */
  LOCAL_UPSTREAM_MIXED_AUDIO = 5,
};
/**
 * The recording mode.
 */
enum MediaRecorderMode {
  /**
   * Non-transcoded recording: Records the original audio/video streams without re-encoding.
   * This mode has lower CPU usage and faster processing.
   */
  RECORDER_MODE_NON_TRANSCODED = 0,
   
  /**
   * Transcoded recording: Re-encodes the audio/video streams during recording.
   * This mode allows for quality optimization and format conversion.
   */
  RECORDER_MODE_TRANSCODED = 1,
};
/**
 ...
 */
 struct AudioConfiguration {
  ...
  int sampleRate;
  /**
   * The recording content. See `AUDIO_RECORDING_CONTENT_TYPE`.
   */
  AUDIO_RECORDING_CONTENT_TYPE audioRecordingType;
  ...
  AudioConfiguration()
      : sampleRate(32000),
        audioRecordingType(MIXED_ALL_AUDIOS),
        recordingChannel(1) {}

  AudioConfiguration(const char* file_path, int sample_rate, int channel)
      : sampleRate(sample_rate),
        audioRecordingType(MIXED_ALL_AUDIOS),
        recordingChannel(channel) {}

  AudioConfiguration(const char* file_path, bool enc, int sample_rate,
          AUDIO_RECORDING_CONTENT_TYPE type, int channel)
      : sampleRate(sample_rate),
        audioRecordingType(type),
        recordingChannel(channel) {}

  AudioConfiguration(const AudioConfiguration& rhs)
      : sampleRate(rhs.sampleRate),
        audioRecordingType(rhs.audioRecordingType),
...
struct MediaRecorderConfiguration {
  ...
  int fps;
  /**
   * The video source just for out channel recorder
   */
  agora::rtc::VIDEO_SOURCE_TYPE videoSourceType;

  /**
   * The recording mode. See \ref agora::rtc::MediaRecorderMode "MediaRecorderMode"
   */
  MediaRecorderMode recordingMode;

  /**
   * The audio configuration. See \ref agora::rtc::AudioConfiguration "AudioConfiguration"
   */
  ...
  MediaRecorderConfiguration()
      : storagePath(NULL),
        ...
        recordingMode(RECORDER_MODE_NON_TRANSCODED),
        audioConfiguration(AudioConfiguration()) {}
