...
struct MixedAudioStream {
  ...
  MixedAudioStream(AUDIO_SOURCE_TYPE source, const char* channel)
    : sourceType(source),
      channelId(channel) {}
...
struct LocalAudioMixerConfiguration {
   ...
   */
  const MixedAudioStream* audioInputStreams;
  ...
  /**
   * Whether to start mixing without any source audio tracks.
   * - `true`: Start mixing without any source audio tracks.
   * - `false`: (Default) Start mixing with the source audio tracks.
   */
  bool mixingWithoutSource;

  LocalAudioMixerConfiguration() : streamCount(0), audioInputStreams(nullptr), syncWithLocalMic(true), mixingWithoutSource(false) {}
};
