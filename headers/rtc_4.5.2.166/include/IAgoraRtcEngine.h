...
struct ChannelMediaOptions {
  ...
  */
  ...
  /**
   * Whether to publish the local camera relay video track.
   * - `true`: Publish the video track of local camera relay video track.
   * - `false`: (Default) Do not publish the local camera relay video track.
   * 
   * warning: updateChannelMediaOptionsEx is not supported for this parameter.
   */
  Optional<bool> publishCameraRelayTrack;
  /**
...
#define SET_FROM(X) SetFrom(&X, change.X)
      ...
      SET_FROM(publishCameraRelayTrack);
...
#define END_COMPARE()
      ...
      ADD_COMPARE(publishCameraRelayTrack);
...
#define REPLACE_BY(X) ReplaceBy(&X, replace.X)
        ...
        REPLACE_BY(publishCameraRelayTrack);
