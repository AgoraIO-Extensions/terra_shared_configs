...
struct AdvanceOptions {
  ...
  AdvanceOptions() : encodingPreference(PREFER_AUTO), compressionPreference(PREFER_LOW_LATENCY) {}

  AdvanceOptions(ENCODING_PREFERENCE encoding_preference,
                 COMPRESSION_PREFERENCE compression_preference)
      : encodingPreference(encoding_preference), compressionPreference(compression_preference) {}
...
struct VideoDenoiserOptions {
  ...
  enum VIDEO_DENOISER_LEVEL {
    ...
    /**
     * Enhanced video noise reduction. `STRENGTH` prioritizes video noise reduction quality over
     * reducing performance consumption. The performance consumption is higher, the video noise
     * reduction speed is slower, and the video noise reduction quality is better. If `HIGH_QUALITY`
...
enum STREAM_SUBSCRIBE_STATE {
   ...
   *   - Calls `muteRemoteAudioStream(true)`, `muteAllRemoteAudioStreams(true)` or
   * `setDefaultMuteAllRemoteAudioStreams(true)` to stop receiving the remote audio streams.
   *   - Calls `muteRemoteVideoStream(true)`, `muteAllRemoteVideoStreams(true)` or
   * `setDefaultMuteAllRemoteVideoStreams(true)` to stop receiving the remote video streams.
   */
